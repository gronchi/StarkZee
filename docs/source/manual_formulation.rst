...
Substituting into Eq. :eq:`eq:Ct` gives the explicit correlation function
...
where :math:`H_A` is the field-free magnetic Hamiltonian (Section 3.2 ) and :math:`V_E(F,\mu)` is the linear Stark interaction (Section 3.4 ). Equations :eq:`eq:resolvent_broadened` and :eq:`eq:liouvillian_field` together give the field-dependent :math:`q`-polarized profile at a given configuration :math:`(F,\mu)`:
...
Sections 3.2 –3.8  detail each ingredient of Eq. :eq:`eq:field_profile`; Section 3.10  extends this framework to include ion dynamics via the FFM.
...
The angular factors :math:`\langle l_l, m_{l,l} | T_q^{(1)} | l_u, m_{l,u} \rangle` follow from the Wigner-Eckart theorem [edmonds]_. They are exactly the rank-1 spherical-tensor matrix elements already written for the Stark coupling in Eqs. :eq:`eq:Tq0`–:eq:`eq:Tqm1`, now evaluated between the lower and upper manifolds: :math:`q=0` gives the :math:`\pi` component (:math:`\Delta m_l = 0`), :math:`q=+1` the :math:`\sigma^-` component (:math:`\Delta m_l = -1`), and :math:`q=-1` the :math:`\sigma^+` component (:math:`\Delta m_l = +1`).
...
Three alternatives are available for the dynamical factor :math:`G(\Delta\omega)` in Eq. :eq:`eq:gamma_k`, all sharing :math:`\langle r^2_\mathrm{intra}\rangle_n`, the same :math:`G_n = C_n` strong-collision constants, and :math:`\kappa_m` as defined above.
