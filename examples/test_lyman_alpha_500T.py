#!/usr/bin/env python3
"""
Test: C VI Lyman-alpha Stark-Zeeman Splitting at B = 500 T
===========================================================

Variant of the Figure 3 benchmark (DOI: 10.1063/5.0058552) at higher field.
...
Energy range ±0.50 eV around E0 to accommodate the larger Zeeman splitting.
The pi component is multiplied by -1 for visual separation.
"""
...
def run_test():
    ...
    # Energy grid: ±0.50 eV around E0 (wider than 100 T case)
    detuning_grid = np.linspace(-0.2, 0.2, 600)
    energies_ev   = E0 + detuning_grid

    print("=" * 70)
    print("test_lyman_alpha_500T: C VI Lyman-alpha — B = 500 T")
    ...
    ax.plot(detuning_grid, (sig_plus + sig_minus)/(sig_plus + sig_minus).max(), linewidth=2.2,
            label=r'$\sigma_+$  ($q = +1$)')
#    ax.plot(detuning_grid, ,linewidth=2.2,
#            label=r'$\sigma_-$  ($q = -1$)')
    ax.plot(detuning_grid, -pi,      linewidth=2.2,
