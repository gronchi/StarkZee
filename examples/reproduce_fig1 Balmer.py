...
reproduce_fig1.py
=================
Reproduces Figure 1 of Ferri, Peyrusse & Calisti (2021):
  Balmer-series lines for H (Z=1), Ne = 10^23 m^-3, Te = 5 eV,
  B = 100 T, 500 T, 1000 T — transverse observation.

Each sub-panel shows:
  solid line  — with  quadratic (diamagnetic) Zeeman
  dashed line — without quadratic Zeeman

The quadratic Zeeman wings on Hα become clearly visible at B ≥ 500 T:
  σ+ wing : 3p(ml=+1)→2s  appears ~5–7 meV beyond the main 3d cluster
  σ− wing : 3p(ml=−1)→2s  appears ~5–7 meV nearer E0 than the 3d cluster
These wings are absent in the dashed (no-QZ) curves.
"""
...
if __name__ == "__main__":
    ...
    wavelengths_nm  = np.linspace(370.0, 720.0, 10000)
    ...
    fig.suptitle(
        r"Stark-Zeeman Balmer-series lines (H, $N_e=10^{23}$ m$^{-3}$, $T_e=5$ eV)"
        "\n"
        r"Solid = with quadratic Zeeman, Dashed = without",
        fontsize=13)

    colors = ["tab:blue", "tab:blue"]
    B_labels = ["B = 0 T", "B = 0 T"]
    ...
    for ax, B, label, color in zip(axes, B_list, B_labels, colors):
        ...
        ax.plot(wavelengths_ang, nq, "-", color=color,
