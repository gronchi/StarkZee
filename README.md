...
*Generated by [`examples/model_comparison.py`](examples/model_comparison.py). StarkZee resolves the Stark-Zeeman triplet that the field-free models (Voigt, Stehlé, Stehlé-parameterized) cannot — Lomanowski has no magnetic-field treatment and is disabled here. See [Built-in Reference Models](docs/manual.tex) for the full comparison table.*
