...
def apply_doppler_broadening_logE(energies_ev, profile, Ti_ev, A_emitter):
    """Apply thermal Doppler broadening via log-energy-space convolution.

    The Doppler kernel width is ``σ(E') = E' × v_th/c``, which depends on the
    source frequency.  In log-energy coordinates ``u = ln(E)`` the kernel
    becomes a fixed-width Gaussian:

        G̃(u − u') = exp(−(u − u')² / (v_th/c)²)

    so a standard FFT convolution applies exactly (to first order in v_th/c,
    which holds for all non-relativistic emitters).  No single line-center E₀
    is assumed; every source point E' carries its own Doppler width.

    Compare with :func:`apply_doppler_broadening`, which uses a fixed width
    ``Δλ_D = λ₀ × v_th/c`` centred at the grid midpoint.

    Parameters
    ----------
    energies_ev : ndarray, shape (N,)
        Photon energy grid [eV].  Need not be uniform.
    profile : ndarray, shape (N,)
        Spectral profile [eV⁻¹].
    Ti_ev : float
        Ion temperature [eV].
    A_emitter : float
        Atomic mass number of the emitting species (1 for H, 2 for D, 3 for T).

    Returns
    -------
    ndarray, shape (N,)
        Doppler-broadened profile [eV⁻¹], with total intensity conserved.

    Notes
    -----
    Jacobian: the Gaussian in u-space is applied to ``P̃(u) = P(E) × E``
    (so that ``∫ P̃ du = ∫ P dE``), then the result is divided by E to
    recover units of eV⁻¹.
    """
    mc2_ev = A_emitter * _M_P * C_LIGHT**2 / E_CHARGE
    v_th_over_c = np.sqrt(2.0 * Ti_ev / mc2_ev)

    # np.interp requires increasing xp; energy grids converted from wavelengths
    # are decreasing, so we sort, compute, then restore original order.
    flip = energies_ev[-1] < energies_ev[0]
    if flip:
        energies_ev = energies_ev[::-1]
        profile = profile[::-1]

    # Map to uniform log-E grid; apply Jacobian P̃(u) = P(E) × E
    u = np.log(energies_ev)
    u_uniform = np.linspace(u[0], u[-1], len(energies_ev))
    profile_u = np.interp(u_uniform, u, profile * energies_ev)

    # Fixed-width Gaussian kernel in log-E: width = v_th/c (1/e half-width)
    x = u_uniform - np.mean(u_uniform)
    kernel = np.exp(-x**2 / v_th_over_c**2)

    # FFT convolve, then undo Jacobian: P_D(E) = P̃_D(u) / E
    result_u = convolve_fft(u_uniform, profile_u, kernel)
    result = np.interp(u, u_uniform, result_u / np.exp(u_uniform))
